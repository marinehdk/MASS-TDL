...
- [x] `colcon build --packages-up-to m7_safety_supervisor --packages-skip m4_behavior_arbiter`（容器内）exit 0  ⚠️ R-P6 偏差，待 D3.1 关闭后转为 `--packages-up-to m7_safety_supervisor` 全包 *(T11 step 3, container exit 0)*
- [x] `python3 -m pytest tests/ -q` 仍 **39 passed**（零回归）*(T11 step 1, 39 passed)*
- [⚠️] `ruff check src/ tests/` **0 violations** *⚠️ 49 pre-existing violations from D1.3b 入 PR 描述 known-issues 段；v3.1 patch 未引入新 violation（CI-scoped paths clean）*
- [x] `bash tools/ci/check-doer-checker-independence.sh` 退出码 0 *(T11 step 6, exit 0)*
- [x] `multi_vessel_lint` CI job 通过 *(本地 ruff scan src/sim_workbench/ais_bridge/*.py 0 violations，CI 实跑待 push)*

#### 三项 NEW DoD

- [x] `/src/l3_tdl_kernel/`、`/src/sim_workbench/`、`/src/rl_workbench/` 三子目录存在；`tree src/ -L 2` 输出与 §v3.1.3 拓扑一致 *(T2 + T10 commits a748ffe, befccc6)*
- [x] `git log --follow src/l3_tdl_kernel/m1_odd_envelope_manager/package.xml` 显示历史保留（`git mv` 血缘正确）*(T2 验证: 468 file renames, 0 insertions/deletions)*
- [x] `src/rl_workbench/` 仅含 `.gitkeep` + `README.md`（README 含三层隔离规则原文 + Phase 4 启动条件 + 引用架构 F.4 + 计划 §0.4 决策 3）*(T10 commit befccc6)*
- [x] `ShipMotionSimulator::export_fmu_interface()` 抽象方法签名 commit；header compile 通过 *(T6 commit 50e45dd)*
- [x] `FCBSimulator::export_fmu_interface()` stub 实装（22 变量清单）；`colcon test --packages-select fcb_simulator` 含 `test_fcb_export_fmu_interface` PASSED *(T7 commit e676cb9, 11 tests PASS in T11)*
- [ ] D1.3c owner（V&V FTE role-hat）对 `FmuInterfaceSpec` / `FmuVariableSpec` 签名签字记录入 PR 评论（≤ 5/22）*[PENDING] D1.3c owner sign-off — async parallel, deadline 5/22；user 切换 V&V FTE role-hat 处理*
- [x] `tools/ci/check-rl-isolation.sh` 新建；`bash tools/ci/tests/test-rl-isolation.sh` 5 TC 全 pass *(T8 commit a4243b4 + fix 20cd97d, 5 TC PASS=7 FAIL=0 in T11 step 8)*
- [x] `.gitlab-ci.yml` `rl_isolation_lint` job 追加（`allow_failure: false`）；feature branch pipeline 触发该 job 绿勾 *(T9 commit 095a406；feature branch 本地无 remote，CI 实跑待 push)*
- [x] `tools/ci/demo/violation_fixture.cpp` + Demo Charter §v3.1.10 操作脚本就位；本地 `≤ 30s` 重现 exit 1 → exit 0 *(T12 commit e7d10ee，本地 dry-run < 30s 通过)*
- [x] 关闭 finding ID：**G P0-G-1 (a) v3.1 追加项**（RL 隔离目录 + FMI 接口存根）+ 计划 v3.1 §0.4 决策 1 + 决策 3 D1.3a 部分

#### 文档与版本同步

- [x] 本 spec §v3.1 章节合入 `01-spec.md`，所有 DoD 复选框打 ✅；commit 由 v3.1 补丁同 PR 提交 *(本 commit T13)*
- [ ] PR 描述附 5 条回归命令（colcon / pytest / ruff / doer-checker / rl-isolation）的实际输出截图或日志 *[PENDING] PR 创建时附 T11 step 1/2/6/7/8 输出*
- [ ] follow-up 标记入 GitLab issue：P-1（m4 package.xml）/ P-2（m8_hmi_bridge 空目录已删，无 follow-up，可标 CLOSED）/ P-3（allowlist 滥用月度复核）/ R-P6 收尾路径 *[PENDING] PR 创建时同步 P-1/P-3/R-P6 issues*
