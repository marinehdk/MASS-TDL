...
async def configure(self, scenario_id: str) -> LifecycleResult:
    ...
    scenario_yaml = await self._store.get_scenario_yaml(scenario_id)
    if scenario_yaml is None:
        raise ScenarioInjectionError(
            f"No scenario YAML found for {scenario_id}"
        )
    try:
        params = self._extract_injection_params(scenario_yaml)
        self._print_injection_summary(scenario_id, params)
...
| `ownShip.initial.sog` (knots → m/s) | `ship_dynamics_node` | `initial_sog` | `5.144` (float, m/s) |
