...
| 版本 | v3.0 |
| 上一版本 | v2.0（2026-05-07，已归档至 `archive/v2.0_2026-05-07_archived.md`）|
| 日期 | 2026-05-08 |
| 计划周期 | 2026-05-08 → 2026-12-31（≈ 8 个月，前 4 个月硬承诺）|
| 架构基线 | `docs/Design/Architecture Design/MASS_ADAS_L3_TDL_架构设计报告.md` v1.1.2（v1.1.3 在本计划 D2.8 stub + D3.8 完整化）|
...
**时间**：2026-05-13 → 2026-06-15（4.5 周）  
**估计人周**：18.0  
**阶段目标**：在写任何业务模块代码前，建立"测试基础设施 + 质量门 + V&V 范式 + 认证证据骨架"。SIL 框架是整个项目的技术基础（最高优先级），V&V Plan + 场景 schema + 覆盖率方法论 是认证基础。**Phase 1 末交付 DEMO-1：Skeleton Live**（哪怕用 mock 也要展示完整骨架）。
...
|---|---|---|---|---|---|
| D1.1 ROS2 工作区 | ▓▓ | ▓▓▓ | | | |
| D1.2 CI/CD 流水线 + PATH-S dry-run | ▓ | ▓▓▓ | ▓▓ | | |
| D1.3a 4-DOF MMG + AIS 管道 | ▓▓▓ | ▓▓▓▓ | ▓▓▓ | | |
| D1.3b 场景管理 + 调试 HMI + 覆盖率报告 | | ▓ | ▓▓▓ | ▓▓▓▓ | ▓▓ |
| D1.3.1 Simulator Qualification Report | | | | ▓▓ | ▓▓ |
| D1.4 编码规范 | | | ▓ | ▓▓ | |
| D1.5 V&V Plan v0.1 [V&V 工程师] | ▓▓ | ▓▓▓ | ▓▓ | | |
| D1.6 场景 schema + 追溯 [V&V 工程师] | | | ▓▓ | ▓▓▓ | ▓ |
| D1.7 覆盖率方法论 [V&V 工程师] | | | | ▓▓ | ▓▓▓ |
| D1.8 cert tracking + ConOps stub | | | | ▓▓ | ▓▓▓ |
...
- [ ] AIS 历史回放 ≥ 1h 真实片段，目标解析率 ≥ 95%
- [ ] `ShipMotionSimulator` 抽象 + FCB plugin（YAML `vessel_class: FCB` 切换）
- [ ] 关闭 finding ID：G P0-G-1 (a) + 多船型 MV-4

**Demo Charter**：6/15 DEMO-1 展示 1h AIS 回放 + 仿真器 own-ship 运动 + ShipMotionSimulator 抽象切换。

---

#### D1.3b · YAML 场景管理 + 调试 HMI + 覆盖率自动报告（D1.3 拆分 b）
...
**Scope**：YAML 声明式场景管理（含 vessel_class / disturbance_model / PRNG seed 字段）；10 基础场景（Head-on × 3, Crossing × 4, Overtaking × 3）批量运行 + JSON 输出；COLREGs 覆盖率 HTML 报告（Rule 5/6/7/8/9/13-17/19 × 通过/失败矩阵）；SIL 调试 HMI（实时 SAT-1/2/3 + 手动场景注入 + ODD 状态可视化；**复用 M8 前端组件库 stub，与 D3.4 共享 → P1-G-5 整改**）。

**DoD**：
- [ ] 10 场景批量自动运行 + HTML 报告生成
- [ ] SIL HMI 实时显示 SAT-1/2/3（M1/M2 stub 接入）
- [ ] 单场景运行 ≤ 60s wall-clock（≥ 600s sim time，P2-E1 整改）
- [ ] 关闭 finding ID：G P0-G-1 (b) + G P1-G-4 + G P1-G-5 + P2-E1

**Demo Charter**：6/15 DEMO-1 中 SIL HMI 是核心舞台。
...
**Scope**：DNV-CG-0264 范式的 V&V Plan，先于任何 SIL 测试启动审批。包含：(a) SIL→HIL→实船 entry/exit gates 完整定义；(b) 端到端 KPI 矩阵（AvoidancePlan P95 ≤ 1.0s / ReactiveOverrideCmd P95 ≤ 200ms / Mid-MPC < 500ms / BC-MPC < 150ms / M7 < 10ms）；(c) functional × performance × failure-response 三类测试覆盖；(d) Adversarial:Nominal:Boundary = 60:25:15 比例声明 + 辩护；(e) ASDR 跨场地一致性约束。

**DoD**：
- [ ] `docs/Design/V&V_Plan/00-vv-strategy-v0.1.md` commit
- [ ] CCS 项目组邮件回执"V&V Plan 框架可接受"
- [ ] 关闭 finding ID：E P0-E1 + E P1-E1/E4/E5 + E P1-E3
...
**Scope**：JSON Schema / Pydantic 强类型场景定义；场景独立 Git repo（或 monorepo 子目录）；每个场景必填 `requirements_traced[]` / `hazid_id[]` / `rule_branch_covered[]` / `vessel_class_applicable[]`；CI 三层集（PR-trigger Smoke 10 / Nightly 200 / Weekly Full 1000+）；命名规范 `<rule>-<odd>-<encounter>-<seed>-<version>.yaml`；场景 ↔ Rule ↔ HAZID ↔ ODD 子域 ↔ 预期 PASS 矩阵。

**DoD**：
- [ ] `docs/Design/SIL/02-scenario-schema.md` + `scenario-traceability-matrix.csv` commit
- [ ] 场景独立 Git repo（或子目录）+ JSON Schema validation 在 CI 强制
- [ ] D1.3b 10 基础场景全部 schema-compliant
- [ ] CI 三层集配置在 GitLab CI 启动（runner 资源 SLA）
- [ ] 关闭 finding ID：G P0-G-2 + G P1-G-6 + P2-G-2 + E P1-E2

**Demo Charter**：6/15 DEMO-1 现场展示 PR 触发 Smoke 10 跑通 + 场景缺陷流（fail → issue → 回归断言）演示。
...
**Scope**：MC/DC（modified condition decision coverage）on Rule 决策树 + Rule × ODD × 扰动立方体（11 Rule × 4 ODD × 5 扰动级别 = 220 cell）+ ISO 21448 SOTIF triggering condition 覆盖（≥ 50 条）+ failure response 覆盖（M7 6 类硬约束 × 10 场景）。

**DoD**：
- [ ] `docs/Design/SIL/03-coverage-metrics.md` commit
- [ ] 覆盖率自动计算脚本（在 D1.3b 覆盖率报告基础上扩展）
- [ ] 关闭 finding ID：E P0-E3 + G P0-G-4

**Demo Charter**：6/15 DEMO-1 展示当前 10 场景的立方体覆盖率热图（即使 220 cell 中只有 ~10 个亮也是合理 baseline）。
...
**Scope**：CCS 9 类证据 ↔ D 编号跟踪表；ConOps v0.1 stub（5–10 页核心场景 + 角色 + 边界 + 操作模式描述，AIP 阶段一头号必需件）。**v3.0 注**：ALARP 完整论证 + SDLC 全表 推到 v1.1.4（B3）；本 D 仅出 ConOps stub + HARA 占位（HARA 实例化在 D2.7）。
...
- [ ] 关闭 finding ID：C P0-C-1 (a) + E P1-E8
...
**Scope（v3.0 修订）**：v2.0 全功能 + DoD 分级（首跑 ≥ 90% / 修缮 ≥ 95% / 仅 D3.6 1000 场景才要 ≥ 98%）。500+ 场景目标不变（用户未取 B1）。

**DoD**：v2.0 D2.4 全部，但首次跑通门槛改为 ≥ 90%（避免假绿灯）。

**Demo Charter**：DEMO-2 中 50 综合场景 live + Rule 13-17 决策链可视化 + COLREGs 覆盖率立方体热图。
...
**Scope（v3.0 修订）**：50 综合场景含 ODD-A/B/C/D × 5 类扰动 cell；端到端延迟 KPI 实测；ASDR 同格式输出（跨 SIL/HIL/实船）。

**DoD**：v2.0 D2.5 全部 + cell 覆盖矩阵 ≥ 80% + 端到端延迟 KPI 通过 + ASDR schema 一致。

**Demo Charter**：DEMO-2 中是核心舞台。

**关闭 finding**：E P0-E5（V&V 独立 owner）+ E P0-E3（部分）+ E P1-E5。
...
**Scope**：(a) 5 名船长访谈（≥ 3 FCB + 1 拖船 + 1 渡船；视线分布 / 扫视模式 / 注意优先级）+ (b) Figma 半保真原型（船长视图 + ROC 视图，对称双角色）+ (c) 桌面可用性测试 n ≥ 5 + (d) ECDIS 集成草案（IHO S-100 / IEC 61174）+ (e) 培训胜任力矩阵草案（基于 STCW 二副 + ROC 增项）+ (f) BNWAS 等价机制设计（IMO MSC.282(86) 对齐）。
...
- [ ] CCS / DNV review 邀请发出
- [ ] 关闭 finding ID：F P0-F-01/F-02/F-04 (stub) + C P1-C-3/C-5（部分）+ B P1-B-06 + F P1-F-03 + D P1-D-08（Y-axis 通告）+ MV-7/8/9/10/11

**Demo Charter**：DEMO-2 中 v1.1.3-stub 全文 review + 4 缺失模块接口图。
...
**Scope（v3.0 修订）**：v2.0 全功能 + (a) active_role 对称双角色完整实现（D0.1 stub → 本 D 实现）+ (b) UI 50Hz 拆 3 档（data_stream_50hz / display_state_4hz / alert_burst_event）+ (c) ToR 自适应矩阵（4 场景）+ (d) BNWAS-equivalent stub（B4 standby）+ (e) Y-axis Reflex Arc → M8 通告通路（ReflexArcActivatedMsg + REFLEX_ACTIVE 子态）+ (f) ECDIS 集成 stub（D2.6 草案落地）+ (g) URGENT_TOR < 30s 子分支（D P1-D-01）+ (h) BackupChannel_DisplayMsg（船端独立 backup，D P1-D-06）+ (i) 5s 强制等待条件分支（D P1-D-03）。

**DoD**：v2.0 D3.4 全部 + 9 项 v3.0 修订全部测试通过 + 资深船长签字 ≥ 1。

**Demo Charter**：
- DEMO-3 核心场景：操作员卧躺 → ToR 唤醒 → 走桥楼 → 60s/120s 内接管 → ASDR 签名
- 6 类 ToR 场景矩阵 live demo
- BNWAS 三级告警（30s 闪灯 → 60s 二副 → 90s 船长）
- Y-axis Reflex Arc 模拟触发 + 红屏全屏蜂鸣

**关闭 finding**：D P0-D-04 + D P1-D-01/D-03/D-06/D-08 + D P2-D-01/D-04 + MV-3/12 + F MISS-9/10/11。
...
**Scope（v3.0 修订）**：v2.0 全功能 + (a) Adversarial:Nominal:Boundary = 60:25:15 比例（D1.5 V&V Plan 锁定）+ (b) HAZID 回填后仅跑"行为分支无关参数"全集（行为分支相关参数推 Phase 4 全集）+ (c) 立方体覆盖（11 Rule × 4 ODD × 5 扰动 ≥ 80% cell hit）+ (d) SOTIF triggering condition 覆盖 ≥ 80%。

**DoD**：v2.0 D3.6 全部 + 立方体覆盖 + SOTIF 覆盖 + 失败场景 report.failures.csv 公开（不掩饰）。

**Demo Charter**：DEMO-3 中 1000+ 场景批量结果 + 立方体覆盖热图 + 失败场景根因分析。

**关闭 finding**：E P0-E3（完整）+ E P0-E2（部分窗口）+ E P1-E3 + G P0-G-4（完整）。
...
**Scope**：在 D2.8 stub 基础上完整化 9 个章节扩展 + 4 缺失模块 stub-only（B4 standby 触发则 Phase 4 完整）+ 算法选型矩阵 + L3 仲裁优先级矩阵 + IDL schema_version + QoS 列。

**DoD**：v1.1.3 主文档 commit + 各模块详设同步 + CCS v1.1.3 邮件记录。

**Demo Charter**：DEMO-3 中 v1.1.3 主文档全文展示 + 仲裁优先级图 + 4 缺失模块接口图。

**关闭 finding**：F P0-F-01/F-02/F-03/F-04（完整）+ B P1-B-06 + F P1-F-02/F-03/F-05/F-06 + 多船型 MV 全部归档。
