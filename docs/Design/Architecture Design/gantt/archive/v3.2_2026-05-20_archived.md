...
| 版本 | **v3.2**（基于 v3.1 + 2026-05-20 Phase 1 进度快照 + DEMO-2 冲刺 GAP patch）|
| 上一版本 | v3.1（2026-05-09，未归档；v3.2 是 v3.1 之上的 inline patch）→ v3.0（2026-05-08）→ v2.0（2026-05-07，已归档至 `archive/v2.0_2026-05-07_archived.md`）|
| 日期 | 2026-05-20（v3.2 patch；v3.1 锁定 2026-05-09；v3.0 锁定 2026-05-08）|
...
### 0.5 Phase 1 进度快照 + DEMO-2 冲刺 GAP（NEW v3.2，2026-05-20）

> **本节为 v3.2 快照**：基于 2026-05-20 仓库实测核查（核查脚本见修订记录 v3.2），用于回答"对照原计划，Phase 1 关了多少 / DEMO-2 还差多少"。详细 DoD ✅ 标注见各 D 任务正文。

#### Phase 1（5/13–6/15）实测关闭率（按 D 任务）

| D 任务 | 状态 | 一句话证据 | DEMO-2 阻塞？ |
|---|---|---|---|
| D0.1/0.2/0.3 surgical + RFC + 工时表 | ✅ 全闭 | v3.0 已记，原 [x] 保留 | — |
| D1.1 ROS2 工作区 + IDL | 🟡 部分 | 8 包 + 25 msg 骨架在；仅 16 含 `schema_version`，mock publisher 在 archive | 🟡 中（M8 SAT-2/3 msg schema 强相关）|
| D1.2 CI/CD + PATH-S | 🟡 部分 | `.gitlab-ci.yml` 9 阶段 + `.clang-tidy.path-s` 在；`check-doer-checker-independence.sh` 缺 | 🟢 不阻塞 DEMO-2 |
| D1.3a MMG + AIS | 🟡 部分 | `ship_dynamics` 4-DOF RK4 真实；AIS 回放工具 / `ShipMotionSimulator` 抽象 / RL 隔离 colcon 边界 未验 | 🟢 |
| D1.3b.1 Imazu-22 + Cerberus | 🟡 部分 | **scenarios/imazu-01..22 全 22 在 + `.imazu22_sha256_manifest.yaml` frozen ✅**；双语言验证缺 | 🟡 |
| D1.3b.2 AIS-driven scenario | 🔴 未做 | 无 Kystverket/NOAA pipeline | 🟡（DEMO-2 Charter 引为现场环节，需降级或推迟）|
| D1.3b.3 Web HMI + ENC + foxglove | 🟡 部分 | MapLibre + foxglove_bridge ✅；S-57 MVT + Apache Arrow replay + GIF batch 缺；**SAT-2/3 WS handler 缺（双端真空，见下）** | 🔴 高（DEMO-2 核心可视化）|
| D1.3c FMI bridge / libcosim / dds-fmu | 🟡 部分 | Humble 容器 ✅；libcosim CMake 在但 dds-fmu 未集成 | 🟢（DEMO-2 不依赖 FMI 端到端）|
| D1.3.1 Simulator Qualification | 🔴 stub 空壳 | 文件存在但无 3 参考解 / 100 次重跑数据 | 🟡（CCS 现场展示用）|
| D1.4 编码规范 | 🔴 | 仓库根无 `coding-standards.md` | 🟢 |
| D1.5 V&V Plan v0.1 | ✅ 完成 | `00-vv-strategy-v0.1.md` 含 SIL latency budget + RL rebound + DNV toolchain entry 三新子节 | — |
| D1.6 场景 schema + farn | 🔴 stub 空壳 | `02-scenario-schema.md` 文件在但内容缺；farn/ospx 未集成 | 🟡 |
| D1.7 覆盖率方法论 + 6 维度评分 | 🔴 stub 空壳 | `03-coverage-metrics.md` 文件在但 rubric 缺；DEMO-2 D2.4 "6 维度评分 per-scenario 输出" 直接挂空 | 🔴 高 |
| D1.8 cert + ConOps | ✅（stub 结构完整）| `cert-evidence-tracking.md` + `01-conops-v0.1.md` 双 stub 已 commit | — |

**Phase 1 关闭率粗算**：12 项 D 任务中 **2 项 ✅ 完整 + 6 项 🟡 部分 + 4 项 🔴 未做**（剔除 D0 已闭）。距 DEMO-1 (6/15) 还 ~3.5 周。

#### TDL Kernel 模块实测（基于源码行数 + grep `publish`）

| 模块 | 主节点 LOC | 真实发布 ROS topic | DEMO-2 状态 |
|---|---|---|---|
| M1 ODD Manager | 1669 | ✅ /sil/m1/odd_state | 🟡 待 D2.1 完整（FMEDA stub + ToR 自适应矩阵）|
| M2 World Model | 1860 | ✅ /sil/m2/world_state（CPA/TCPA 真）| 🟡 待 D2.2（sog/扇区/intent/env 4 项 v3.0 修订）|
| M3 Mission Mgr | 1488 | ✅ | 🟡 待 D2.3 |
| **M4 Behavior Arbiter** | 476 | ⚫ ivp_solver.cpp 在但未发布 SAT-2 ivp_contributions | 🔴 **DEMO-2 阻塞** |
| **M5 Tactical Planner** | (CasADi 集成规划)| ⚫ 未发 SAT-3 trajectory_candidates | 🔴 **DEMO-2 阻塞** |
| M6 COLREGs | 976 | ✅ Rule 5-19 推理真实；未发 SAT-2 colregs_chain 5 层结构 | 🟡 |
| M7-core Safety | (assumption_monitor.cpp 在)| ✅ Safety_AlertMsg | 🟡 待 D3.3a/b |
| M8 HMI Bridge | (C+++FastAPI 双进程)| ✅ /sil/own_ship/target_vessel/module_pulse/asdr_event/scoring/lifecycle/control_cmd/env 12 topic；**❌ 缺 /sil/sat2_data + /sil/sat3_data + /sil/sotif_metrics** | 🔴 **DEMO-2 阻塞** |

#### Web HMI Screen 3 双端真空（DEMO-2 P0 风险）

**症状**：前端 [SimulationMonitor.tsx](web/src/screens/SimulationMonitor.tsx) 已 mount `IvpRiskGradientLayer` / `MpcTrajectoryLayer` / `ColregsRationaleTree` / `SotifMonitorStrip` 全部 Engineer 视图组件壳；Zustand store [sat.ts](web/src/store/sat.ts) 数据结构齐备；但 [useFoxgloveLive.ts](web/src/hooks/useFoxgloveLive.ts) TOPIC_MAP 仅 12 个 SAT-1 级 topic，**SAT-2/SAT-3/SOTIF 没有 WS handler，组件全部渲染 null**。

**根因双端**：(a) producer 侧 M8 没发对应 topic；(b) consumer 侧 useFoxgloveLive 没订阅 handler。

#### DEMO-2 (7/31) 冲刺 GAP 排序（按阻塞强度）

| Pri | 工作项 | 跨 D | 估算 | 说明 |
|---|---|---|---|---|
| **P0** | M8 增发 `/sil/sat2_data` + `/sil/sat3_data` + `/sil/sotif_metrics` ROS topic + IDL schema（含 `ivp_contributions[]` / `colregs_chain[5]` / `trajectory_candidates[13]` / `sotif_metrics{6}`）| D1.1 + D3.4 | 1.5pw | 解双端真空必要条件 |
| **P0** | useFoxgloveLive TOPIC_MAP 增 3 个 handler + Zustand updater | D1.3b.3 | 0.5pw | 解双端真空必要条件 |
| **P0** | M4 IvP Solver 实装并发布 `ivp_contributions[]`（行为字典 5 行为权重 + 胜出行为 + 4Hz）| D3.1 | 4.0pw（原 4.0pw 计划）| 提前 Phase 3 至 Phase 2 末 |
| **P0** | M5 BC-MPC 13 候选弧线 + Mid-MPC 90s 主轨迹发布到 SAT-3（即使初版用线性化 Nomoto + N=12 兜底）| D3.2 | 6.5pw（原 6.5pw 计划）| 提前 Phase 3 至 Phase 2 末 |
| **P0** | M6 5 层 decision rationale 序列化输出到 SAT-2 colregs_chain | D2.4 | 0.5pw（D2.4 内吸收）| M6 推理已真，只缺序列化 |
| **P0** | D1.7 6 维度评分 rubric 文档完整化（Hagen 2022/Woerner 2019）| D1.7 | 1.5pw（原计划）| D2.4 验收依赖 |
| **P1** | M7 SOTIF 5 类假设违反指标聚合到 `/sil/sotif_metrics` | D3.3b | 1.0pw（D3.3b 内吸收）| 监控带数据源 |
| **P1** | D1.3.1 Simulator Qualification 真实跑 3 参考解（旋回/Z 字/直线减速）+ 100 次重跑 | D1.3.1 | 1.0pw（原计划）| CCS 现场展示 |
| **P1** | D1.6 场景 schema 完整化（maritime-schema 扩展 + traceability 矩阵）| D1.6 | 2.0pw（原计划）| DEMO-2 50 综合场景前置 |
| **P1** | D2.7 HARA ≥30 危险源 + FMEDA M1 表 | D2.7 | 2.5pw（原计划）| CCS 中期意见会议必需 |
| **P1** | ARPA 表 BRG/RNG 列 wire 到 M2 输出 | D2.2 | 0.2pw | 现硬编码 "—" |
| **P1** | RmVectorLayer 新增组件 | D3.4 | 0.5pw | Engineer 视图完整性 |
| **P2** | D1.4 coding-standards.md 落地 | D1.4 | 0.5pw（原计划）| CCS Cert 路径要 |
| **P2** | D1.3b.2 AIS-driven scenario 工具 | D1.3b.2 | 4.0pw（原计划）| DEMO-2 Charter 有该环节，可降级为"用 D1.3b.1 内置 22 场景演示"|
| **P2** | ROC viewMode 实装 | D3.4 | 1.0pw | DEMO-3 可吸收 |

**P0 冲刺合计** ≈ **14.5pw**（含 M4/M5 原 Phase 3 工时前置 10.5pw + SAT 桥 + rubric 4.0pw）；距 7/31 剩 ~10 周；单人产能 1.85pw/周 ⇒ 双人 ~18.5pw/周，理论可吸收但前提是 M4/M5 owner 早开工不延后 1 天。

#### 计划是否需要调整？

**建议作为 v3.2 patch 同步如下变更**（已在本文档内 inline 修订）：

1. **D2.4 Scope 显式加"5 层 colregs_chain SAT-2 序列化"**（原计划只在 D3.4 暗含）
2. **D2.5 SIL 集成 DoD 加"`/sil/sat2_data` + `/sil/sat3_data` + `/sil/sotif_metrics` 3 个 topic 端到端验证"**
3. **D3.1 + D3.2 开工时点前移到 7/13**（已与原甘特对齐，强调不可滑期）
4. **D3.4 Scope 拆出"SAT 桥接接口与前端 handler 对齐"为子条目**，提到 Phase 2 末（v3.2 新增条目）
5. **新增 D1.3b.3-suppl**：useFoxgloveLive SAT-2/3 handler 落地（0.5pw，归 M8 前端 owner）
6. **D1.3b.2 推迟到 Phase 4**（B4 类缓冲，DEMO-2 改用 D1.3b.1 内置 22 Imazu 演示，不影响硬验收）
7. **v3.1 §0.4 D1.3c FMI bridge** 维持降级范围（仅 own-ship FMU），DEMO-2 不展示端到端 FMI
...
**DoD**：
- [~] 25 + 12 .msg 与 v1.1.2 §15.1 完全对齐 + schema_version 字段 ⚠️ **v3.2 实测：l3_msgs 25 条 .msg 已建，仅 16 条含 schema_version；DEMO-2 P0：补齐剩余 9 条 + 新增 SAT-2/SAT-3/SOTIF 3 个 .msg**
- [ ] Mock publisher 以正确频率/字段发布所有外部接口消息 ⚠️ **v3.2 实测：mock publisher 仍在 archive/sil_v0，未迁移到当前 src/**
...
**DoD**：
- [~] CI 5 阶段全部通过（非 `|| true`）⚠️ **v3.2 实测：`.gitlab-ci.yml` 已含 9 阶段流水线（>5 已扩展）**
- [ ] Docker 镜像可复现构建（固定版本号）
- [ ] PATH-S CI 独立性检查在 D1.x 全部通过 dry-run 后切强制（截止 6/15）⚠️ **v3.2 实测：`.clang-tidy.path-s` 已 commit；`check-doer-checker-independence.sh` 脚本未见**
...
**DoD**：
- [x] 仿真器连续 1h 无崩溃；停船误差 ≤ 10%（vs 解析参考解）✅ **v3.2 实测：`ship_dynamics/mmg_model.py` 4-DOF RK4 实装；单元测试通过**
- [ ] AIS 历史回放 ≥ 1h 真实片段（Kystverket / NOAA MarineCadastre 开放数据），目标解析率 ≥ 95% ⚠️ **v3.2 实测：AIS 回放工具未见**
- [ ] `ShipMotionSimulator` 抽象 + FCB plugin（YAML `vessel_class: FCB` 切换）⚠️ **v3.2：抽象层不明确**
- [~] **v3.1 NEW**：`/src/l3_tdl_kernel/` 与 `/src/sim_workbench/` 独立 colcon 包；CI lint rule 拒绝 sim_workbench 内 `#include` l3_tdl_kernel 头文件 ⚠️ **v3.2：两目录结构存在但 CI lint rule 未验证**
...
**DoD**：
- [~] 22 Imazu + 10 自编场景全部 `maritime-schema` v0.2.x schema-compliant ⚠️ **v3.2 实测：22 Imazu 场景全在 `scenarios/imazu-01..22/`；10 自编场景未见；maritime-schema 兼容性未验**
- [x] `imazu22_v1.0.yaml` SHA256 frozen，PR-trigger CI fast gate 必跑全 22 ✅ **v3.2 实测：`scenarios/.imazu22_sha256_manifest.yaml` 已 commit**
- [ ] Python `cerberus` + C++ `cerberus-cpp` 同 schema 双语言验证通过 ⚠️ **v3.2：双语言验证未见**
...
**DoD**：⚠️ **v3.2 实测：本 D 全部未启动，建议触发 v3.2 §0.5 调整建议 #6 推迟到 Phase 4**
...
**DoD**：
- [ ] S-57 → MVT 管线对 Trondheim + SF Bay 出 vector tiles，浏览器加载 < 2s ⚠️ **v3.2 实测：S-57 MVT 管线未见**
- [x] `foxglove_bridge` 接 ROS2 → 50Hz own/target state 实时推 web，无丢包 ✅ **v3.2 实测：`useFoxgloveLive.ts` 12 topic 实测可用，subprotocol foxglove.sdk.v1**
- [x] MapLibre HMI 加载 + own-ship sprite + 1 target sprite 同屏 @60FPS ✅ **v3.2 实测：`web/package.json` maplibre-gl 集成；SilMapView 渲染本船+target**
- [x] CPA/TCPA/Rule/Decision 信息块视觉风格与原 HTML 一致 ✅ **v3.2 实测：ThreatRibbon/ArpaTargetTable 已实装（CPA/TCPA 真实订阅）**
- [x] M1-M8 pulse + ASDR 日志实时显示 ✅ **v3.2 实测：ModulePulseBar + AsdrLedger 真实订阅**
...
- [ ] **v3.2 NEW（DEMO-2 P0）**：useFoxgloveLive TOPIC_MAP 增 `/sil/sat2_data` + `/sil/sat3_data` + `/sil/sotif_metrics` 3 handler；前端 IvpRiskGradientLayer / MpcTrajectoryLayer / ColregsRationaleTree / SotifMonitorStrip 4 组件接通真数据（解 "Screen 3 双端真空"，见 v3.2 §0.5）
- [ ] **v3.2 NEW**：ARPA 表 BRG/RNG 列接 M2 输出（现 hardcoded "—"）
- [ ] **v3.2 NEW**：RmVectorLayer 新增（设计文档列但未实装）
...
**DoD**：
- [~] `libcosim` + `ospx` 安装 + 编译通过（Ubuntu 22.04 + ROS2 Humble + Python 3.11 容器）⚠️ **v3.2 实测：libcosim 在 CMakeLists 规划；Humble Docker 容器迁移已完成（feat/d1.3b.3-gap-015-sil-fixes）；Python 3.11 显式验证缺**
- [ ] `dds-fmu` mediator 集成 + own-ship MMG FMU loaded by libcosim 在 SIL 运行 1h 无崩溃 ⚠️ **v3.2：dds-fmu 未集成**
...
**DoD**：
- [~] `docs/Design/SIL/01-simulator-qualification-report.md` v0.1 commit ⚠️ **v3.2 实测：文件已创建但为 STUB 空壳；DEMO-2 P1 必须填充 3 参考解实测数据**
...
**DoD**：见 v2.0 D1.4，未变更。⚠️ **v3.2 实测：`.clang-tidy` 已 commit；`coding-standards.md` 仓库根缺失（仅 archive 副本）；DEMO-2 之前可不强阻塞，CCS Cert 路径 11 月 AIP 前必须完整化**
...
**DoD**：
- [x] `docs/Design/V&V_Plan/00-vv-strategy-v0.1.md` commit（含 v3.1 新增 §SIL latency budget + §RL rebound + §DNV toolchain entry）✅ **v3.2 实测：00-vv-strategy-v0.1.md + 2026-05-12-vv-phase1-artifacts.md 均 commit；三新子节齐备**
...
**DoD**：
- [~] `docs/Design/SIL/02-scenario-schema.md`（含 maritime-schema 扩展规约 + `metadata.*` 字段表）+ `scenario-traceability-matrix.csv` commit ⚠️ **v3.2 实测：02-scenario-schema.md 文件已建但内容空壳；DEMO-2 P1 必须完整化**
- [ ] 场景独立 Git repo（或子目录）+ maritime-schema validation 在 CI 强制
- [~] D1.3b.1 22 Imazu + 10 自编场景全部 maritime-schema-compliant ⚠️ **v3.2：22 场景在但 schema 兼容未验**
- [ ] `farn` v0.4.2+ 安装 + 1100-cell case folder dry-run（实际跑量 D3.6）⚠️ **v3.2：farn 未集成**
- [ ] `ospx` 集成 + own-ship FMU 系统结构配置可生成 ⚠️ **v3.2：ospx 未集成**
...
**DoD**：⚠️ **v3.2 实测：本 D 是 DEMO-2 P0 阻塞项之一（D2.4 评分依赖）**
- [~] `docs/Design/SIL/03-coverage-metrics.md` commit（含 v3.1 新增 §6维度评分 rubric + §Monte Carlo 段 + §60:25:15 自我辩护）⚠️ **v3.2：文件已建但 STUB 空壳**
...
**DoD**：
- [x] `docs/Design/Cert/cert-evidence-tracking.md` commit ✅ **v3.2 实测：文件已建（stub 结构完整）**
- [x] `docs/Design/ConOps/01-conops-v0.1.md` commit ✅ **v3.2 实测：文件已建（stub 结构完整）**
...
**DoD**：v2.0 D2.1 全部 + FMEDA M1 表 v0.1 + ToR 自适应矩阵 + Capability Manifest 集成。⚠️ **v3.2 实测：M1 主节点 1669 LOC + odd_state topic 发布；FMEDA stub / ToR 自适应矩阵 / Capability Manifest 集成全部未见，DEMO-2 P0**
...
**DoD**：v2.0 D2.2 全部 + sog/扇区/intent/env 4 项 v3.0 修订全部测试通过。⚠️ **v3.2 实测：M2 主节点 1860 LOC + world_state topic 真实发布（CPA/TCPA Eigen 计算）；4 项 v3.0 修订（sog 校验 / OVERTAKING 扇区 / intent_distribution / env sanity check）需逐项验证；DEMO-2 P1 加 BRG/RNG 输出供 ARPA 表使用**
...
**DoD**：v2.0 D2.3 全部 + 海流误差告警链路验证 + L1/L2 双订阅独立性测试。⚠️ **v3.2 实测：M3 主节点 1488 LOC + 主 topic 发布；海流误差告警 / L1/L2 双订阅独立性测试需逐项验**
...
**⚠️ v3.2 实测 + 新增 DEMO-2 P0 子条目**：
- [~] M6 主节点 976 LOC + COLREGs Rule 5-19 推理真实
- [ ] **v3.2 NEW（DEMO-2 P0）**：M6 5 层 decision rationale 序列化输出到 SAT-2 `colregs_chain[5]`（→ M8 转发 → 前端 ColregsRationaleTree）— M6 推理已真，仅缺 schema 序列化，估算 0.5pw 在 D2.4 内吸收
- [ ] 6 维度评分依赖 D1.7 rubric 文档（D1.7 当前 stub，必须先完整化）
- [ ] AIS-derived encounter / `ncdm_vessel` / trafficgen 50→200 — 依赖 D1.3b.2（v3.2 推迟到 Phase 4），DEMO-2 改用 D1.3b.1 内置 22 Imazu 演示
...
**⚠️ v3.2 NEW（DEMO-2 P0）**：
- [ ] `/sil/sat2_data` + `/sil/sat3_data` + `/sil/sotif_metrics` 3 个 ROS topic 端到端验证（producer M4/M5/M7 → M8 桥 → foxglove_bridge → 前端 store → Engineer 视图组件渲染非 null）
- [ ] 该 3 topic IDL schema 与前端 [sat.ts](web/src/store/sat.ts) 类型定义一一对齐
...
**DoD**：⚠️ **v3.2 实测：HF 外包合同 6/16 起，本 D 当前未启动；DEMO-2 (7/31) 验收前必须完成 5 份访谈 + Figma + 可用性测试**
...
**DoD**：⚠️ **v3.2 实测：`docs/Design/Safety/` 目录无内容；DEMO-2 P1 强阻塞（CCS 中期意见会议必需），安全工程师外包应于 5/15 启动**
...
**DoD**：
- [x] 架构 v1.1.3-stub commit（D3.8 完整化）✅ **v3.2 实测：架构主文件版本号已升 v1.1.3-pre-stub；28 章节齐备**
...
**⚠️ v3.2 实测 + DEMO-2 P0 提前要求**：
- [~] M4 主节点 476 LOC + `ivp_solver.cpp` 存在（IvP 自实现方向 per RFC-009）；但 ROS topic 发布未验证
- [ ] **DEMO-2 P0**：M4 IvP 行为字典 5 行为权重 + 胜出行为 输出到 `/sil/sat2_data.ivp_contributions[]` @ 4Hz（前端 IvpRiskGradientLayer 8 方向梯度向量数据源）
- [ ] **DEMO-2 P0**：D3.1 开工时点不可滑期，按原甘特 7/13 起，目标 7/26 提供至少 stub 输出（哪怕固定权重表）以供 7/31 DEMO-2 现场跑通
...
**⚠️ v3.2 实测 + DEMO-2 P0 提前要求**：
- [ ] M5 CasADi/IPOPT 集成规划存在；BC-MPC 13 分支 + Mid-MPC 90s 实装未见
- [ ] **DEMO-2 P0**：M5 BC-MPC 13 候选弧线 + Mid-MPC 主轨迹发布到 `/sil/sat3_data.trajectory_candidates[]`（前端 MpcTrajectoryLayer 渲染数据源）；DEMO-2 (7/31) 阶段允许初版用线性化 Nomoto + N=12 兜底（DEMO-3 升级到 CasADi/IPOPT 完整）
- [ ] **DEMO-2 P0**：D3.2 开工时点不可滑期，按原甘特 7/13 起，目标 7/27 提供 SAT-3 trajectory_candidates 输出（哪怕初版无最优化，13 个几何分支）
...
**DoD**：v2.0 D3.3 主体（IEC 61508 + 6 类硬约束）+ Doer-Checker 三量化矩阵 commit + FMEDA M7 表（≥ 20 失效模式）+ PATH-S CI 0 violation。⚠️ **v3.2 实测：M7 独立 PATH-S 路径无证据；Safety_AlertMsg 真实发布；DEMO-2 P1 至少能让 M7 独立进程心跳可见**
...
**⚠️ v3.2 实测 + DEMO-2 P1 提前要求**：
- [~] `assumption_monitor.cpp` 已 commit（feat/d3.3b-m7-sotif 分支，1 commit ahead）
- [ ] **DEMO-2 P1**：M7 SOTIF 6 指标聚合到 `/sil/sotif_metrics` topic（前端 SotifMonitorStrip 6 行进度条数据源），DEMO-2 阶段允许 stub 值，DEMO-3 升级为真实滑窗
...
**⚠️ v3.2 实测 + DEMO-2 P0 拆条**：
- [x] M8 双进程拓扑（C++ ROS + Python FastAPI）+ 12 个 SAT-1 级 topic 真实发布 ✅
- [x] ToR Modal ≥ 2s 物理锁 ✅ **v3.2 实测：[TorModal.tsx](web/src/components/TorModal.tsx) hold-to-confirm 已实装**
- [ ] active_role 对称双角色 `active_role.py` 存在但完整对称未验
- [ ] ToR 自适应矩阵（4 状态 30/60/90/120s）— 未实装
- [ ] **DEMO-2 P0（拆出至 Phase 2 末）**：M8 增发 `/sil/sat2_data` + `/sil/sat3_data` + `/sil/sotif_metrics` 3 个 topic + IDL schema 与前端 [sat.ts](web/src/store/sat.ts) 一一对齐；该子项不可推到 Phase 3，原 D3.4 整体延 8/24 但 SAT 桥接 7/31 前必须就绪
...
**DoD**：⚠️ **v3.2 实测：RFC-007 + RFC-009 当前仍在 archive 目录，未迁入 `docs/Design/Cross-Team Alignment/`；建议 D3.9 启动时一并迁移**
...
| v3.1 | 2026-05-09 | 2 次 SIL 深度研究 + 用户决策 Q1/Q2/Q3 后整合：选项 D 混合架构 + DNV 工具链 3 MUST + ROS2 Humble + RL 隔离三层 + maritime-schema + Imazu-22 + AIS-driven scenario + Hagen 2022 / Woerner 2019 评分 + Web HMI。新增 D1.3a 注 + D1.3b 拆 b.1/b.2/b.3 + D1.3c FMI bridge NEW；累计缺口扩至 -32~-34 pw，用户授权"工时缺口不用担心，核心目标完整实现"。|
| **v3.2** | **2026-05-20** | **Phase 1 进度快照 patch（用户要求对照原计划逐项标注实测完成度，瞄准 DEMO-2 冲刺）。核心变更**：(1) 新增 §0.5 Phase 1 进度快照 + DEMO-2 冲刺 GAP 表（12 项 D 任务实测关闭率 + 8 模块发布状态 + Web HMI Screen 3 双端真空诊断 + 15 项 P0/P1/P2 冲刺工时排序）；(2) 各 D 任务 DoD 列表打勾 ✅ 已闭 / [~] 部分 / ⚠️ 实测注，**已确认 ✅ 完整闭口**：D0 全部、D1.5 V&V Plan v0.1（含 SIL latency budget + RL rebound + DNV toolchain entry 三新子节）、D1.8 cert tracking + ConOps stub、D2.8 v1.1.3-pre-stub、D1.3b.1 22 Imazu frozen hash、D1.3a MMG 4-DOF RK4、D1.3b.3 foxglove_bridge + MapLibre + ConningBar/ThreatRibbon/ModulePulse/AsdrLedger 真实订阅、D3.4 ToR Modal ≥2s 物理锁；**DEMO-2 P0 阻塞清单**：D1.3.1 / D1.6 / D1.7 三 stub 空壳 + D2.7 HARA 未启 + M4 IvP 发布 + M5 BC-MPC 发布 + M8 SAT-2/3/SOTIF 桥接 + useFoxgloveLive 3 handler；(3) D2.4 Scope 新增"M6 5 层 colregs_chain SAT-2 序列化"子条目；D2.5 DoD 新增"SAT-2/3/SOTIF 3 topic 端到端验证"；D3.1/D3.2 加 DEMO-2 P0 提前要求（M4/M5 不可滑期，stub 输出 7/26/27 前到位）；D3.4 拆出 SAT 桥接子项必须 7/31 前就绪（不与 D3.4 整体 8/24 同步）；D1.3b.2 AIS-driven 推迟到 Phase 4（DEMO-2 改用 D1.3b.1 内置 22 Imazu 演示，B4 类缓冲）；D1.3b.3 DoD 加 useFoxgloveLive SAT 3 handler 新条目（解 Screen 3 双端真空）；(4) 工时缺口不重算（沿用 v3.1 用户授权）；本 patch 不改架构 v1.1.3-pre-stub 头表、不改甘特节奏、不重排 DEMO milestone，仅补充 DEMO-2 阻塞点的执行优先级与提前要求。|
