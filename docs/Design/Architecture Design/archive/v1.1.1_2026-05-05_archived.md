...
| 版本 | **v1.1.1**（基于 v1.1 + 2026-05-05 复审 patch — 关闭 v1.1 修订引入的 6 条 finding）|
| 日期 | 2026-05-05 |
| 状态 | 正式设计稿（满足 CCS i-Ship 入级加严 A 档；可启动详细功能设计）|
| 目标船型 | 45m FCB（首发）· 多船型可扩展 |
| 目标船级社 | CCS（中国船级社）单一入级 |
| 自主等级覆盖 | D2 – D4 全谱 |
| 修订基线 | v1.0 → v1.1（ADR-001/002/003 + 35 条 patch）→ v1.1.1（6 条新 finding 关闭 patch）|
| 关闭 finding（v1.1.1 累计）| P0=5 / P1=21（v1.1: 18 + v1.1.1: 3）/ P2=18（v1.1: 15 + v1.1.1: 3）/ P3=2 deferred |
...
*本报告版本 v1.1.1。基于 v1.1（2026-05-05 修订完成）+ 2026-05-05 复审 patch（关闭 6 条 v1.1 引入的 finding）。简化 Phase 3+6 复审落点：A 档（加严版）— 全表综合分 2.89 / P0=0。所有 [初始设计值] / [TBD-HAZID] / [HAZID 校准] 项详见附录 E，须在 FCB 实船试航前通过 HAZID 工作组关闭。文档变更须经系统架构评审委员会批准。*
