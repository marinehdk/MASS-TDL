...
| 版本 | **v1.1**（基于 v1.0 + 2026-04-30 审计 patch + ADR-001/002/003 修订）|
| 日期 | 2026-05-05 |
| 状态 | 正式设计稿（待架构评审委员会复审）|
| 目标船型 | 45m FCB（首发）· 多船型可扩展 |
| 目标船级社 | CCS（中国船级社）单一入级 |
| 自主等级覆盖 | D2 – D4 全谱 |
| 修订基线 | v1.0（2026-04-29）+ `docs/Design/Architecture Design/audit/2026-04-30/` 审计交付物 |
| 关闭 finding | P0=5 / P1=18 / P2=15 / P3=2（详见 `audit/2026-04-30/05-audit-report.md`）|
...
权重（FCB 特定设计值，待 HAZID 校正 [F-P0-D6-015 + F-P2-D6-018]）：
  w_E = 0.4（环境条件最难干预）
  w_T = 0.3（任务条件可通过减速/停航干预）
  w_H = 0.3（人机责任可通过通信恢复改善；H 轴权重须以 Veitch 2024 TMR 实证 [R4] 校准）
```
...
| **L3 Checker 否决率** [F-P2-D3-036 新增] | X-axis Checker 否决次数 / 100 周期 | > 20（即 20% 否决率） | 升级 SOTIF 告警："COLREGs 推理可信度下降"；触发降级到 D2 评估 |
...
### 11.6 MRM 命令集（v1.1 新增 — ADR-001）[F-P0-D3-001 + F-P1-D6-004]

M7 ARBITRATOR 不再动态生成轨迹，仅触发**预定义 MRM 命令集**（M7 持 VETO 权 + 命令集索引）：

| 索引 | 命令 | 触发场景 | 执行细节 |
|---|---|---|---|
| **MRM-01** | 减速至 4 kn 维持航向 | 一般 PERF 退化 / SOTIF 告警 | 30 秒内完成减速；保持当前航向；通知 M8 → ROC |
| **MRM-02** | 停车（速度 → 0）| Checker 多次否决 / 连续假设违反 | 推力归零；漂航；通知 ROC 介入 |
| **MRM-03** | 紧急转向 | CPA 急剧恶化（PERF 紧急级）| 基于威胁方向，预定义反向避让（COLREGs Rule 17 兜底）|
| **MRM-04** | 抛锚序列 | 港内 / 近岸 + 系统失能 | 仅适用近岸场景 + 水深允许；含锚位预选 |

> MRM 命令集的具体参数（4 kn / 30 秒 / 反向角度等）须 FCB 实船 HAZID 校准。
>
> M7 输出 `Safety_AlertMsg.recommended_mrm` = MRM-{01|02|03|04} 索引；M5 解析索引并执行预定义指令；M7 不持 MPC / 规划逻辑（决策四独立性约束）。
...
- **独立性约束**：X-axis 与 M7 通过独立总线通信，**不共享代码 / 库 / 数据结构**（决策四独立性约束）
...
### 11.9 人工接管时 L3 内部行为（v1.1 新增）[F-P2-D6-037]
...
- **回切**（Arbiter 解除接管）：M5 重启 MPC（积分项重置）；M7 重启 SOTIF；M1 恢复 ODD 周期；ASDR 标记 "override_released" 事件
...
# [F-P0-D3-002 + F-P2-D3-036 新增 — M7 ↔ X-axis Checker 协调]
message CheckerVetoNotification {
    timestamp    stamp;
    string       checker_layer;    # "L2"|"L3"|"L4"|"L5"
    string       vetoed_module;    # 被否决的 Doer 模块名
    string       veto_reason;
    Trajectory   nearest_compliant; # X-axis 提供的回退方案（仅供 M7 知情，不直接执行）
}
...
| **M1, M2, M4, M6, M7 → ASDR** | ASDR_RecordMsg | 事件 + 2 Hz | 决策追溯日志（JSON + SHA-256 签名）**[F-P1-D4-033 新增]** |
...
- HAZID 校准任务清单（所有 "[HAZID 校正]" 注脚的参数）
- v1.2 / spec part 2 议题（CSP 多目标消解 / FCB 高速在线辨识 / 工业系统对比）
- v1.1 复审建议（简化 Phase 3 + Phase 6 复审）

---

*本报告版本 v1.1。基于 v1.0（2026-04-29）+ 2026-04-30 审计交付物 + ADR-001/002/003 修订。所有 [TBD] / [HAZID 校正] 项须在对应开发里程碑前通过 CCS 项目组征询和内部 HAZID 研讨会关闭。文档变更须经系统架构评审委员会批准。*
