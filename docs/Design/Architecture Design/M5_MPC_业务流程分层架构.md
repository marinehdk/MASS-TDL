...
| **L1a OCP 建模(规格冻结,Step2 重新定义 VR-07)** | **hard-never-in-idxsh 原则**已落 codegen+wrapper+adversarial 测试;**`kGIdxCpaHard` global slot** 新增完整 wiring(BL-08);**box live 落地**(stage0 x0 equality 不被 box 覆盖);**ROT 来源修正**(GNC ODD);**heading/ROT schedule 分离**;**terminal contract** 显式定义(BL-10);**grid physical-time map 原则**(BL-13);**prefix 段 hard row relax 安全边界显式标注依赖 L1b D1**(L1a 测试限定 prefix_active_k=0) | ⬜ 待实施 | nh 抉择(BL-09)挪 **Step5 DESIGN-IT-TWICE**;DP-01 row 布局实施依赖 Step5 |
...
| **L1b OCP 建模(依赖 Q4 + L1a 留的公式回填)** | k_head 公式 + ample-time 下界 `t_latest_safe`(BL-12);CPA suffix-hard schedule(DP-07,DP-01×DP-08 耦合解耦);min-alt reachability b'(VR-03 修正);prefix CPA NO_SAFE_PLAN+M7(VR-04,路径 A+D1);direction row reachability schedule;**Q4 σ conditional**(IPOPT,BL-15/BUG-L1-01 扩大);**VR-03 b' 在线 k ≥ oracle k cross-check CI** | ⬜ 待实施 | Step5 nh 抉择 + L1a-spec-freeze GATE + Q4 决策(已明确:acados 路径 A + IPOPT σ conditional + D1 witness) |
...
| **DP-02 box live** wrapper 每 solve 每 stage 重发 lbx/ubx(消费 live heading/speed);**stage0 保持 x0 equality 不被 box 覆盖** | L2 | L1a-spec-freeze | L1a GATE | BL-10 terminal contract | ⬜ 待实施 |
| **DP-02 ROT 来源修正** ROT 来自 GNC ODD(`mid_mpc_node.cpp:902-916`)而非 M4 | L2 | L1a-spec-freeze | L1a GATE | 无(Codex 纠正 [R24]) | ⬜ 待实施 |
| **DP-02 terminal contract** 显式定义 NBXN=0 是"terminal 不在安全 claim"还是"纳入 NBXN"(BL-10) | L1+L2 | L1a-spec-freeze | L1a GATE | BL-10 闭环 | ⬜ 待实施 |
| **DP-02 heading/ROT schedule 分离** heading soften,ROT 全 stage hard(adversarial T-DP02-3) | L2 | L1a-spec-freeze | L1a GATE | DP-08 k_head(L1b) | ⬜ 待实施 |
| **DP-08 原则** grid physical-time map:所有 schedule 先按物理秒定义再映射各自 backend grid,禁裸 k parity(IPOPT/acados off-by-one) | L1+LX | L1a-spec-freeze | L1a GATE | BL-13 原则层闭环 | ⬜ 待实施 |
| **DP-08 k_head 公式** k_head + ample-time 下界 `t_latest_safe`(双量) | L1 | **L1b**(原 C1a) | L1b GATE | BL-12 Step3/Step5 | ⬜ 待实施 |
| **DP-03** min-alt reachability b'(保守因子 + oracle cross-check) | L1 | C1b | L1b GATE | Q4 已确认 | ⬜ 待实施 |
| **DP-04** prefix CPA NO_SAFE_PLAN+M7(路径 A+D1) | L1+L5 | C1b | L1b GATE | Q4 已确认 | ⬜ 待实施 |
| **DP-07** prefix CPA row 处理 + CPA suffix-hard schedule(DP-01×DP-08 耦合解耦) | L1 | C1b | L1b GATE | Q4 + DP-08 k_head | ⬜ 待实施 |
| **direction row** reachability schedule(§4.4 k=0 soften 镜像) | L1 | C1b | L1b GATE | GNC Q7 | ⬜ 待实施 |
| **Q4 σ conditional** IPOPT prefix CPA σ 吸收修复 + D1 witness(两路径同源) | L1(IPOPT) | C1b | L1b GATE | BL-15 + BUG-L1-01 | ⬜ 待实施 |
