...
| **L1b OCP 建模(依赖 Q4 + L1a 留的公式回填)** | k_head 公式 + ample-time 下界 `t_latest_safe`(BL-12,✓);CPA suffix-hard schedule(DP-07,✓);min-alt reachability b'(VR-03 修正);prefix CPA NO_SAFE_PLAN+M7(VR-04,✓);direction row reachability schedule(✓);**Q4 σ conditional**(IPOPT,✓);**VR-03 b' 在线 k ≥ oracle k cross-check CI** | ✓ **L1b 核心完成(4 commits a16c14397..c1981d47c)** | min-alt b' 待实施(独立于其他 L1b 项) |
