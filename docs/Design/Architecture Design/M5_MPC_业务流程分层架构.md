...
| **L2 求解准备** | runtime heading/speed/ROT box 每 stage 落地(VR-02);codegen re-run(NSH=0/NP=211 生效);heading/ROT schedule 分离;seed/warm-start F1 前向传播;nsh=0 验证(slack guard + CPA hard floor + d_min telemetry) | ✓ **完成(commit pending;codegen NSH=0/NP=211/NH=20 ✅;box live heading/ROT schedule 分离 ✅;d_min fix for softened CPA rows ✅;17/17 acados + 55/55 IPOPT + 4/4 CPA hard floor tests)** | L1+L2 GATE |
...
| **DP-01 子项** `kGIdxCpaHard` global slot 新增完整 wiring(enum + pack + codegen SX 镜像 + np_global 154→155 + 三方 hash) | L1+L2 | L1a-spec-freeze | L1a GATE | BL-08 闭环 | ✓ **完成(commit fe251260b;slot 154 追加,target block 不动;codegen re-run ✅ NSH=0/NP=211,三方 hash 生效)** |
...
| **DP-02 heading/ROT schedule 分离** heading 按 k_head_earliest 延迟硬化,ROT 全 stage hard(无 schedule);solver box live 中 per-stage 区分 | L2 | L2(原 L1b,DP-08 k_head 已就绪后提前实施) | L2 GATE | DP-08 k_head(L1b) ✓ + compute_reachability_schedule | ✓ **完成(commit pending;box live 块中 heading 按 k_head_earliest 延迟,ROT/speed 始终硬绑定;已验证 17/17 acados + 55/55 IPOPT tests)** |
