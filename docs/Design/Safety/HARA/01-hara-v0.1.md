...
| H-06 | CAT-A | M2 | AIS/雷达目标融合失败：虚假目标或漏报目标 | 碰撞避险基于错误世界模型；避碰动作方向错误 | C3 | F2 | SIL 1 | SIF-05 | ODD-B | DEG | AIS + 雷达 + 视觉融合冗余；M7 数据一致性交叉检查 | Medium | SIF-05 D2.7 新增；[HAZID 校准] 融合误差阈值；v0.1: SIF-05 基线 SIL 1（风险图 C3×F2→SIL 2，D3.3 LOPA 确认） |
...
| H-12 | CAT-B | M3 | 重规划触发阈值过高：危险场景下未及时请求重规划 | 船舶持续执行次优计划；碰撞/险近风险累积 | C2 | F3 | SIL 1 | SIF-06 | ODD-B | DEG | M3 CPA 告警阈值 + M1 ODD 状态变化强制重规划 | Medium | [HAZID 校准] CPA 阈值；v0.1: SIF-06 基线 SIL 1（风险图 C2×F3→SIL 2，D3.3 LOPA 确认） |
...
| H-15 | CAT-B | M7 | M7 Checker 误 VETO 正确 Doer 决策（False Positive）：不必要 MRC 触发 | 不必要操作员接管；OOTB 场景；可用性下降 | C1 | F3 | SIL 0 | SIF-01 | ALL | NOM | Checker 阈值合理性定期校验 + DEMO-2 数据采集（误 VETO KPI）| Low | 安全失效（Safe Failure）；v0.1: SIL 0（风险图 C1×F3→SIL 1，安全失效免 SIL）；可用性指标监控 |
...
| H-20 | CAT-C | M1 | 模块健康误报：M2 心跳短暂延迟被判为失效 → 不必要 ToR 触发 | 不必要操作员接管请求；OOTB 场景 | C1 | F3 | SIL 0 | SIF-01 | ALL | NOM | 心跳容忍窗口（3 次丢失确认）+ 平滑降级逻辑 | Low | 安全失效；v0.1: SIL 0（风险图 C1×F3→SIL 1，安全失效免 SIL）；可用性 KPI 记录 |
| H-21 | CAT-C | M2 | 目标船速度估算延迟（> 2s 旧数据）：TCPA 计算偏差 | 避碰时机错误；险近 | C2 | F3 | SIL 1 | SIF-05 | ODD-B | DEG | M2 stamp 时效验证 + M1 TCPA 输入过时惩罚系数 | Medium | 参见 FMEDA FM-M1-14；v0.1: SIF-05 基线 SIL 1（风险图 C2×F3→SIL 2，D3.3 LOPA 确认） |
...
| H-24 | CAT-D | M8 | ToR 请求未送达 ROC（Shore Link 丢失）：ROC 未收到接管请求 | 接管时窗内 ROC 未响应；MRM 被迫触发 | C2 | F2 | SIL 2 | SIF-04 | ODD-D | DEG | Shore Link 双通道冗余 + MRM 自动回退（ADR-001 §11.6）+ M8 本地告警 | Medium | [HAZID 校准] Shore Link 可靠性要求；v0.1: SIL 2（风险图 C2×F2→SIL 1，SIF-04 基线 SIL 2 保守升级） |
...
| H-27 | CAT-D | M8 | Shore Link 时钟漂移（ROC 与船端时基不同步）：ToR 时间窗口计算错误 | ROC 感知接管时窗与船端实际不同；MRC 时机错误 | C2 | F3 | SIL 1 | SIF-08 | ODD-D | DEG | NTP/PTP 时间同步监控 + M1 时间戳合法性检查 | Medium | [HAZID 校准] 时基精度要求；v0.1: SIF-08 基线 SIL 1（风险图 C2×F3→SIL 2，D3.3 LOPA 确认） |
...
| H-29 | CAT-E | M8 | SAT-2 预测缺失：ROC 不了解系统未来 60s 意图 | ROC 无法提前判断接管必要性；决策延迟 | C2 | F3 | SIL 1 | SIF-08 | ALL | DEG | M8 forecast(Δt) + uncertainty() 强制输出（CMM 三接口）| Low | ADR §3.4 CMM 接口；M8 必须实现；v0.1: SIF-08 基线 SIL 1（风险图 C2×F3→SIL 2，D3.3 LOPA 确认） |
...
| H-32 | CAT-E | M8 | SAT-3 理由缺失：ROC 看到决策结果但无法理解原因 | ROC 无法验证决策合理性；信任度下降 + 错误介入 | C2 | F3 | SIL 1 | SIF-08 | ALL | NOM | M8 rationale() 强制字段（架构 §3 CMM + IEC 62443 + IMO MASS §4.3）| Low | CMM rationale() 接口；D2.6；v0.1: SIF-08 基线 SIL 1（风险图 C2×F3→SIL 2，D3.3 LOPA 确认） |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
...
**SIF 覆盖验证（T6 架构师评审验证，2026-05-22）：**
