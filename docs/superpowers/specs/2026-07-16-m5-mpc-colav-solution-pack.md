...
本方案包管**裁决依据**(做什么的算法/约束/时域选择),执行**顺序、依赖、各 phase scope** 见:

> `specs/2026-07-17-m5-mpc-p0-p7-roadmap.md`(M5 MPC 避碰重构 P0–P7 执行路线图)

该路线图同时落地了 L3→L4 GNC 契约设计(2026-07-17)对本方案的跨树修订:
- **VR-06b**: horizon 360s→**1200s**(P4,覆盖完整避碰生命周期 900s)
- **VR-07b**: 废弃人工参考轨迹→相对跟踪 t_b + Huber;废除终端 C10/C11(P2)
- **VR-02**: 淘汰 TailBuilder(P2 输出流程)
- **VR-01**: 新 TimedTrajectory 输出 / **VR-05**: preflight 删硬编码(契约兑现项,见路线图 §5.5)

**两套 phase 编号须区分**: 本方案 P0–P7(M5 MPC 核心) ≠ GNC 迁移路线 P1–P4(VR-11,L3↔L4 接口迁移)。详见路线图 §2。
