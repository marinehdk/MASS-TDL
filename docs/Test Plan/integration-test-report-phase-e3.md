...
| 优先级 | 任务 | 负责人/分支 |
|---|---|---|
| P0 | 合并本 PR → main（wave-4/integration） | wave-4/integration |
| P0 | 修复 F-INT-001 + F-INT-002（topic 前缀不一致），分支 `fix/integration-topic-mismatches` | fix/integration-topic-mismatches |
| P0 | 修复 M8 ToR callsite，解除 `test_hil_rule19_degraded` ToR 路径阻断，分支 `fix/m8-tor-callsite` | fix/m8-tor-callsite |
| P1 | 在实物 FCB 硬件上执行 100 场景套件，目标：≥95/100 通过率（需 F-INT-001 已修复） | Team-HIL |
| P1 | 在实物 FCB 硬件上采集时序基准（Mid-MPC / BC-MPC / Reflex Arc / L4 切换 P99 实测值） | Team-HIL |
| P2 | HAZID RUN-001 完成 → v1.1.3 参数回填（2026-08-19） | docs/Design/HAZID/ |
| P2 | Phase E4 实船试航启动（计划 2026-06） | docs/Test Plan/ |
