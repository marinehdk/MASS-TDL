...
| 0.2 | 2026-04-26 | 架构组 | v2.0 升级：提升为 Actuator Envelope Checker（Deterministic Checker L5 层实例）；增加物理一致性校验；增加与硬线仲裁器的双层保护架构说明；增加 ASDR 违规记录 |
| 0.3 | 2026-04-26 | 架构组 | v3.0 升级：增加 Power Management 功率约束检查；增加 Blackout Prevention 防黑船逻辑；增加三层功率保护架构说明 |
