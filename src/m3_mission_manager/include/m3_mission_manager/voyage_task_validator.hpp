...
class VoyageTaskValidator {
 ...
 private:
  // 7 individual check methods (each <= 40 lines)
  ...
  // 7. check_optimization_priority -- must be a known value
  ...
  ValidationResult check_optimization_priority(const std::string& optimization_priority) const;
