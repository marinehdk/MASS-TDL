...
class IvPSolverTest : public ::testing::Test {
 ...
 TEST_F(IvPSolverTest, SingleBehaviorNoConstraint) {
   ...
   ASSERT_EQ(f.set_pieces({{85.0, 95.0, 14.0, 16.0, 1.0}}), M4ErrorCode::kOk);
 ...
 TEST_F(IvPSolverTest, TwoCompatibleBehaviorsWeightedSum) {
   ...
   ASSERT_EQ(transit_f.set_pieces({{85.0, 95.0, 14.0, 16.0, 1.0}}), M4ErrorCode::kOk);

   IvPFunctionDefault colreg_f;
   ASSERT_EQ(colreg_f.set_pieces({{80.0, 100.0, 12.0, 18.0, 0.8}}), M4ErrorCode::kOk);
 ...
 TEST_F(IvPSolverTest, HeadingConstraintNarrowsSolution) {
   IvPFunctionDefault f;
   ASSERT_EQ(f.set_pieces({{0.0, 180.0, 5.0, 15.0, 1.0}}), M4ErrorCode::kOk);
 ...
 TEST_F(IvPSolverTest, RelaxPreservesOriginalWhenLevelZero) {
 ...
 TEST_F(IvPSolverTest, RelaxLevelOneClearsCpa) {
   IvPHardConstraints original;
   original.cpa_safe_m = 1852.0;
 ...
 TEST_F(IvPSolverTest, RelaxLevelTwoClearsHeadingRanges) {
   IvPHardConstraints original;
 ...
 TEST_F(IvPSolverTest, SolveWithFallbackWorksWithNormalConstraints) {
 ...
 TEST_F(IvPSolverTest, SolveWithFallbackDegradesWhenTightlyConstrained) {
   IvPHeadingDomain hd(5.0);
   IvPSpeedDomain sd(0.0, 22.0, 1.0);
   auto strategy = std::make_unique<WeightedSumCombination>();
   IvPSolver solver(hd, sd, std::move(strategy), std::chrono::milliseconds(500));
