...
- **概念查询、规范条款、算法对比** 先用 `/nlm-ask` 查本项目笔记本：
  - 5 个 DOMAIN（按 `.nlm/config.json` `domain_notebooks` 路由）：`safety_verification` / `ship_maneuvering` / `maritime_human_factors` / `maritime_regulations` / `colav_algorithms`
  - 2 个 v3.0 NEW global notebooks（按 UUID 直查；nlm-setup 暂不支持 --create-domain）：
    - SIL/HIL Platform Engineering: `968983cc-56f4-4203-90fb-02b0739a0594`（DNV OSP / Kongsberg HIL / FMI 2.0 / scenario coverage / simulator V&V / tool qualification）
    - Maritime Cybersecurity & Onboard Networks: `6cb48f75-4bd5-4d61-b161-ff83377ab013`（IACS UR E26/E27 / OT zoning / DDS-Security / mTLS / ECDIS chart signing / ASDR HMAC）
