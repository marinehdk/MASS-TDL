...
private:
    ...
    void odom_callback(const Odometry::SharedPtr msg)
    {
        ...
        json ship_state = {
            ...
            {"roll_deg",       roll_deg},
            {"pitch_deg",      pitch_deg},
            {"roll_rate_dps",  RAD2DEG(msg->twist.twist.angular.x)},
            {"pitch_rate_dps", RAD2DEG(msg->twist.twist.angular.y)},
            {"yaw_rate_dps",   RAD2DEG(msg->twist.twist.angular.z)},
            {"x_local",        x},
            {"y_local",        y},
            {"mode",           "simulation"},
            {"timestamp",      timestamp}
