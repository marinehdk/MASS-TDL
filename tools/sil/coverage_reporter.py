...
def generate_report(results_dir: Path, output_path: Path) -> None:
    ...
    # D1.7: Also generate 1100-cell coverage heatmap alongside the detailed report
    heatmap_path = output_path.with_name(
        output_path.stem.replace("coverage_report", "coverage_heatmap") + ".html"
    )
    try:
        from scenarios_coverage import generate_heatmap as _gen_heatmap

        traceability_csv = results_dir / "traceability.csv"
        if traceability_csv.exists():
            _gen_heatmap(traceability_csv, heatmap_path)
    except Exception as exc:
        import sys
        print(
            f"[WARNING] coverage heatmap generation skipped: {exc}",
            file=sys.stderr,
        )
