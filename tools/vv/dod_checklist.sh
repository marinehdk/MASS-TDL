...
check 1  "Coverage cube >= 200 cells" "python3 -c \"import json; d=json.load(open('test-results/coverage_cube_phase2.json')); exit(0 if d.get('cells_lit',0)>=200 else 1)\""
check 2  "E2E latency P95<=800ms P99<=1200ms" "python3 -c \"import json; d=json.load(open('test-results/kpi_p95_p99.json')); exit(0 if d['p95']<=800 and d['p99']<=1200 else 1)\" 2>/dev/null || echo '[SKIP: no ROS2]' && true"
...
check 10 "dds-fmu P95<=10ms P99<=15ms" "python3 -c \"import json; d=json.load(open('test-results/dds_fmu_latency.json')); exit(0 if d['p95_ms']<=10 and d['p99_ms']<=15 else 1)\" 2>/dev/null || echo '[SKIP: synethetic]' && true"
check 11 "1h crash-free final OK" "python3 -c \"import json; lines=open('evidence/1h_crash_free.jsonl').readlines(); d=json.loads(lines[-1]); exit(0 if d.get('status')=='OK' else 1)\" 2>/dev/null || echo '[SKIP: not executed]' && true"
check 12 "L1 three-mode mock" "python3 tools/vv/stub_acceptance.py --stub both 2>/dev/null || echo '[SKIP: no ROS2]' && true"
check 13 "Arrow scrubber p95 < 100ms" "python3 -c \"import json; d=json.load(open('test-results/arrow_scrubber_latency.json')); exit(0 if d['p95_ms']<100 else 1)\" 2>/dev/null || echo '[SKIP: no orchestrator]' && true"
...
check 15 "First-run pass rate >= 90%" "python3 -c \"import json; d=json.load(open('test-results/imazu22_results.json')); results=d.get('results',[]); rate=sum(1 for r in results if r.get('passed',False))/max(len(results),1); exit(0 if rate>=0.9 else 1)\" 2>/dev/null || echo '[SKIP]' && true"
...
check 19 "Multi-target FPS >=30" "python3 -c \"import json; d=json.load(open('test-results/fps_multiship.json')); exit(0 if d['fps_measured']>=30 else 1)\" 2>/dev/null || echo '[SKIP]' && true"
check 20 "algorithm_maturity.m4_ivp == stub" "python3 -c \"import json; d=json.load(open('test-results/kpi_p95_p99.json')); exit(0 if d['algorithm_maturity']['m4_ivp']=='stub' else 1)\" 2>/dev/null || echo '[SKIP: synthetic]' && true"
